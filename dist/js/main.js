...
function open() {
  var text = $(this).attr('id').slice(1);
  if($(this).attr('class') == 'tapeNews__accordion activ') {
  ...
  for (var i = 1; i < 5; i++) {
    if(i != text) {
      ...
      $('#a'+i).removeClass('activ');
    };
  };
};
};
$('.tapeNews__accordion').click(open);
...
for (var i = 0; i < data.length; i++) {
  skills =_.union(skills, _.pick(data[i], 'skills').skills);
};
for(var j = 0; j < skills.length; j++) {
  skills[j] = skills[j].toLowerCase();
};
sortSkills = _.sortBy(skills);
console.log(sortSkills);

var name = [];
var sortName = [];
for (var i = 0; i < data.length; i++) {
  name.push(_.pick(data[i], ['name','friends']));
};
name = _.sortBy(name, 'friends');
for (var i = 0; i < name.length; i++) {
  sortName.push(name[name.length - 1 - i].name);
}
...
var sortFriends = [];
for (var i = 0; i < data.length; i++) {
  friends.push(_.pick(data[i], 'friends'));
};
for (var i = 0; i < friends.length; i++) {
  for (var j = 0; j < (friends[i].friends).length; j++) {
    friendsArray.push((friends[i].friends)[j].name);
  }
}
sortFriends = _.union(friendsArray);
